...
static uint8_t hk_simulator_buttons[JOYSTICK_NUM_MODES] = { 0, 1, 2, 3, 4, 5 };
...
int joystick_set_type(struct joystick *joystick, char *type)
{
    ...
    } else if (!strcmp(type, "hk_simulator")) {
        memcpy(&joystick->buttons, hk_simulator_buttons, sizeof(joystick->buttons));
        memcpy(&joystick->axes, hk_simulator_axes, sizeof(joystick->axes));
    } else {
